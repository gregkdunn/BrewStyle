...
class ContentStore {
	...
	func fetchTagsWithCompletionBlock(block:(result: [TagViewModel]) -> Void ) {
		...
		let query = PFQuery(className: "Tag").fromLocalDatastore().includeKey("category").addAscendingOrder("name")
		query.findObjectsInBackgroundWithBlock {(objects: [PFObject]?, error: NSError?) -> Void in
			if error == nil {
	...
	func fetchSubcategoriesForTag(tag: Tag, block:(result: [BeverageSubCategoryViewModel]) -> Void ) {
		print("Tag - single includes subs")
		
		let query = PFQuery(className: "Tag").fromLocalDatastore().includeKey("BeverageSubCategories").whereKey("objectId", equalTo:tag.objectId!)
		...
		query.findObjectsInBackgroundWithBlock {(objects: [PFObject]?, error: NSError?) -> Void in
			if error == nil {
				if let objects = objects as? [Tag] {
					...
					var subCatVMs : Array<BeverageSubCategoryViewModel> = [BeverageSubCategoryViewModel]()
					if let tagVM = objectVMs.first {
						
						print("got something here... break")
						
					}
					
					block(result : subCatVMs)
				}
			} else {
				print("Error: \(error!)")
			}
