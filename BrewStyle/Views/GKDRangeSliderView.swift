...
class RangeSliderTrackLayer: CALayer {
	weak var rangeSlider: GKDRangeSliderView?
...
class GKDRangeSliderView : UIControl {
	var debug = false
	...
	var thumbTintColor: UIColor = AppColor.LightGray.alpha(0.1) {
		didSet {
	...
	func updateLayerFrames() {
		...
		let newFrame = CGRect(x: bounds.origin.x, y: (bounds.origin.y + bounds.size.height * 0.2) , width: bounds.width, height: bounds.height)
		rangeIndicator.frame = newFrame.insetBy(dx: 0.0, dy: bounds.height / 3)
		rangeIndicator.setNeedsDisplay()
	...
	override func continueTrackingWithTouch(touch: UITouch, withEvent event: UIEvent?) -> Bool {
		...
		if debug {
			print("moved to  \(beginningPoint.x)")
		}
		return true
	}
