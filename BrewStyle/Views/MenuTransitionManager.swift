...
class MenuTransitionManager: UIPercentDrivenInteractiveTransition, UIViewControllerAnimatedTransitioning, UIViewControllerTransitioningDelegate {
    ...
    func animateTransition(transitionContext: UIViewControllerContextTransitioning) {
        ...
        let menuViewController = !self.presenting ? screens.from as! SettingsViewController : screens.to as! SettingsViewController
		...
		if let viewContainer = container {
			viewContainer.addSubview(menuView)
			viewContainer.addSubview(topView)
			viewContainer.addSubview(self.statusBarBackground)
		}
        ...
        UIView.animateWithDuration(duration, delay: 0.0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.8, options: [], animations: {
                ...
                if(transitionContext.transitionWasCancelled()){
                    ...
                    if let window = UIApplication.sharedApplication().keyWindow {
						window.addSubview(screens.from.view)
					}
	...
	func offStageMenuControllerInteractive(menuViewController: SettingsViewController){
	...
	func onStageMenuController(menuViewController: SettingsViewController){
