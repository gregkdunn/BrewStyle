...
class TermListingController: GKDFilterableViewController {
	...
	override func reloadData () {
		...
		ContentStore.sharedInstance.fetchTermsWithCompletionBlock {
			...
			self.data = result//self.sortByCategory(result)
			if result.count > 0 {
				self.loadMoreData()
				self.empty.disable()	
			} else {
	...
	override func createCollectionView () {
		...
		collectionView.registerClass(TermListingCell.self, forCellWithReuseIdentifier: TermCellIdentifiers.term.rawValue)
		collectionView.registerClass(CategoryDetailView.self, forSupplementaryViewOfKind:UICollectionElementKindSectionHeader, withReuseIdentifier: TermCellIdentifiers.header.rawValue)

		empty.configWithIcon(UIImage(named:"pint_stamped"),title:"loading", description: "terms")
...
extension TermListingController : UICollectionViewDataSource {
	...
	func collectionView(collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, atIndexPath indexPath: NSIndexPath) -> UICollectionReusableView {
		...
		let header : CategoryDetailView = collectionView.dequeueReusableSupplementaryViewOfKind(kind, withReuseIdentifier: TermCellIdentifiers.header.rawValue, forIndexPath: indexPath) as! CategoryDetailView
	...
	func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
		let cell = collectionView.dequeueReusableCellWithReuseIdentifier(TermCellIdentifiers.term.rawValue, forIndexPath: indexPath) as UICollectionViewCell
