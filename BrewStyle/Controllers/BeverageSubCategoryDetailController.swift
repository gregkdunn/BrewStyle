...
enum SubCategoryDetailCollectionViewSections: Int {
	...
	static func sectionsForSubCategory(subCategoryVM : BeverageSubCategoryViewModel) -> [SubCategoryDetailCollectionViewSections] {
		if subCategoryVM.isBeer() {
			return SubCategoryDetailCollectionViewSections.beerSections
		} else if (subCategoryVM.isCider()) {
			return SubCategoryDetailCollectionViewSections.ciderSections
		} else {
			//its mead
			return SubCategoryDetailCollectionViewSections.meadSections
		}
	}
	
	static var beerSections : [SubCategoryDetailCollectionViewSections] {
		return [Title, Ranges, OverallImpression, Aroma, Appearance, Flavor, Mouthfeel, Comments, History, CharacteristicIngredients, StyleComparison, EntryInstructions, CommercialExamples, Tags]
	}
	
	static var ciderSections : [SubCategoryDetailCollectionViewSections] {
		return [Title, Ranges, OverallImpression, Aroma, Appearance, Mouthfeel, Comments, EntryInstructions, CharacteristicIngredients, CommercialExamples]
	}
	
	static var meadSections : [SubCategoryDetailCollectionViewSections] {
		return [Title, Ranges, OverallImpression, Aroma, Appearance, Flavor, Mouthfeel, Comments, EntryInstructions, CommercialExamples]
	}
}
...
class BeverageSubCategoryDetailViewController: GKDFilterableViewController {
	...
	var sections : [SubCategoryDetailCollectionViewSections]? = []
	...
	override func createCollectionView () {
		...
		empty.configWithIcon(UIImage(named:"pint_stamped"),title:"loading", description: "beer style")
...
extension BeverageSubCategoryDetailViewController : UICollectionViewDataSource {
	...
	func numberOfSectionsInCollectionView(collectionView: UICollectionView) -> Int {
		if let subCatVM = subCategoryVM {
			sections = SubCategoryDetailCollectionViewSections.sectionsForSubCategory(subCatVM)
			
			if let currentSections = sections {
	...
	func collectionView(collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, atIndexPath indexPath: NSIndexPath) -> UICollectionReusableView {
		if let currentSections = sections,
		   let subCatVM : BeverageSubCategoryViewModel = subCategoryVM {
			let sectionIdentifier = currentSections[indexPath.section]
	...
	func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
		...
		return collectionView.dequeueReusableCellWithReuseIdentifier(SubCategoryDetailCellIdentifiers.Blank.rawValue, forIndexPath: indexPath)
	}
			...
			switch sectionIdentifier {
				case .Title :
					if let subCatVM = subCategoryVM {
						...
						sizingTitleCell.cellWidth = collectionView.bounds.width
				...
				case .Ranges :
					if let subCatVM = subCategoryVM {
						...
						sizingRangesCell.cellWidth = collectionView.bounds.width
				...
				case .Appearance :
					if let subCatVM = subCategoryVM {
						...
						sizingAppearanceCell.cellWidth = collectionView.bounds.width
				...
				case .Tags :
					if let subCatVM = subCategoryVM {
						...
						sizingTagsCell.cellWidth = collectionView.bounds.width
				...
				default :
					if let subCatVM = subCategoryVM {
						...
						sizingDescCell.cellWidth = collectionView.bounds.width
